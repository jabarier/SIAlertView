...
#define CONTENT_PADDING_TOP 34
#define CONTENT_PADDING_BOTTOM 24
#define BUTTON_HEIGHT 35
#define CONTAINER_WIDTH (CGRectGetWidth([UIScreen mainScreen].bounds) - 30)
...
- (void)validateLayout
{
    ...
    if (self.closeButtonVisible) {
        ...
        self.closeButton.frame = CGRectMake(CONTAINER_WIDTH - CLOSE_WIDTH - 10, 10, CLOSE_WIDTH, CLOSE_WIDTH);
    }else {
...
{
    if (self.titleLabel) {
        return [self sizeForLabel:self.titleLabel withText:self.title].height;
        /*
...
- (CGSize)sizeForLabel:(UILabel *)label withText:(NSString *)text{
    CGRect bounds = [text boundingRectWithSize:CGSizeMake(CGRectGetWidth(label.frame), 10000) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:label.font} context:nil];
    return bounds.size;
}
...
- (CGFloat)heightForMessageLabel
{
    ...
    if (self.messageLabel) {
        /*
         ...
         constrainedToSize:CGSizeMake(CONTAINER_WIDTH - CONTENT_PADDING_LEFT * 2,
         maxHeight)
         lineBreakMode:self.messageLabel.lineBreakMode];
         */
        CGSize size = [self sizeForLabel:self.messageLabel withText:self.message];
...
- (void)setupCloseButton {
    if (!self.closeButton) {
        ...
        [closeButton setImage:[UIImage imageNamed:@"icon-close"] forState:UIControlStateHighlighted];
